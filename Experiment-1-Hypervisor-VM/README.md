...
Configured the VM's CPU, RAM, storage, and network.
...
VirtualBox download page Fig 1.1 — Official VirtualBox download page showing platform packages and the Extension Pack.
...
Ubuntu ISO download page Fig 2.1 — Canonical Ubuntu Downloads page for Ubuntu 26.04 LTS Desktop.
...
Create VM wizard Fig 3.1 — Create Virtual Machine wizard: VM name, folder, and ISO image selection.
...
VM overview Fig 4.1 — VirtualBox Manager showing the newly created Ubuntu 26.04 VM and its configuration summary.
...
General settings Fig 5.1 — General settings confirming OS type as Linux / Ubuntu (64-bit).
...
System motherboard settings Fig 5.2 — System > Motherboard tab showing base memory and boot order configuration.
...
System processor settings Fig 5.3 — System > Processor tab showing 3 CPUs allocated to the VM.
...
Display and storage settings Fig 6.1 — Display settings (video memory, monitor count) and Storage controller overview.
...
Display expert mode Fig 6.2 — Expert mode Display settings showing Graphics Controller options.
...
Terminal apt update Fig 7.1 — Terminal inside the running Ubuntu 26.04 VM after executing sudo apt update.
...
Terminal build-essential install Fig 8.1 — build-essential and linux-headers installation/verification inside the VM.
